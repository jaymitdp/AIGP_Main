...
<body class="skin-black-light sidebar-mini">
            ...
            <ul class="nav navbar-nav"  style="width:100%;">
              ...
              <li class="dropdown notifications-menu" title="Available reservior level in units, read user manual for more info." style="width:12%">
                <a href="#" class="dropdown-toggle"  data-toggle="modal" data-target="#reserviorWizard">
                  ...
                  <span class="label label-success" id="reserviorAlert"></span>
                </a>
              </li>
          ...
          <div class="row">
			<div class="col-xs-12">
              <div class="box box-primary">
                ...
                <div class="box-body">
                  <div id="bloodGlucoseGraph" style="height: 350px;"></div>
				</div>
				</div>
				<div class="box box-primary" id="safeDoseInjectionBox" style="display: none;">
                <div class="box-body pad table-responsive">
                  <table class="table table-bordered text-center">
                    <tr>
                      <td>
                      <button id="injectSafeDosageBtn" class="btn btn-block btn-success btn-lg">Inject Safe Dose</button>
                      <input type="hidden" id="isEmergencyCase" value="${sessionScope.isEmergencyCase}"/>
                      </td>
                    </tr>
                  </table>
                </div>
	...
	<script type="text/javascript" src="${pageContext.request.contextPath}/pumpLayout/dist/js/commonMethods.js"></script>
  ...
  $(function () {
  	...
  	var updateData = function(glucoseData)
	{
	  ...
	  if($("#isEmergencyCase").val() == "true")
       	{
       		$('#bloodGlucoseGraph').height("350px");
       		chart.render();
        	$('#safeDoseInjectionBox').show();
       	}
       	if($("#isEmergencyCase").val() == "false")
       	{
       		$('#bloodGlucoseGraph').height("430px");
       		$('#safeDoseInjectionBox').hide();
       		chart.render();
       	}
       	
        $("#isEmergencyCase").change(function () {
            if ($(this).val() == true) {
          	  $('#bloodGlucoseGraph').height("350px");
          	  chart.render();
          	  $('#safeDoseInjectionBox').show();
	 ...
	 $('#reserviorAlert').on('DOMSubtreeModified',function(){
			if(reserviorLevelValue != $('#reserviorAlert').html() && $('#reserviorAlert').html() != "" && $('#reserviorAlert').html() != null && $('#reserviorAlert').html() != "!")
			{
				reserviorLevelValue = $('#reserviorAlert').html();
				alterReserviorIcon();	
			}
		});
  ...
  $('#injectSafeDosageBtn').click(function(){
	  $.ajax({
			type: "POST",
	  			url : './../../glucoseSimulator',
	  			data : {
	  				injectSafeDosage: true
	  			},
