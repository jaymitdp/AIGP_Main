...
public class ContinousGlucoseGenerator{
	...
	public double maxChangeInBloodGlucose, bloodGlucose, idealBloodGlucoseLevel, carbIntake;
	...
	public synchronized void run() {
		...
		{
	       	try {
	       	   if(continousGlucoseGenerator.bloodGlucoseCurrentState == BloodGlucoseCurrentState.INCREASING && ((bloodGlucose < maxChangeInBloodGlucose && bloodGlucose > idealBloodGlucoseLevel) || (pumpConfig.isManualModeOn && pumpConfig.insulinUnitsToConsume <= 0)))
	 	       {
	       	   ...
	       	   if(continousGlucoseGenerator.bloodGlucoseCurrentState == BloodGlucoseCurrentState.INCREASING && ((bloodGlucose >= maxChangeInBloodGlucose && !pumpConfig.isManualModeOn) || (pumpConfig.isManualModeOn && pumpConfig.insulinUnitsToConsume > 0)))
	       	   {
	       		   ...
	       		   pumpConfig.ReserviorLevel = 100 - pumpConfig.insulinConsumed;
	       		   pumpConfig.insulinUnitsToConsume = 0;
	       	   }
	       	   if(continousGlucoseGenerator.bloodGlucoseCurrentState == BloodGlucoseCurrentState.DECREASING && bloodGlucose > 120)
	       	   {
		       ...
		       if(continousGlucoseGenerator.bloodGlucoseCurrentState == BloodGlucoseCurrentState.DECREASING && bloodGlucose <= 85)
		       {
		    	   continousGlucoseGenerator.bloodGlucoseCurrentState = BloodGlucoseCurrentState.INCREASING;
		       }
		       if(continousGlucoseGenerator.bloodGlucoseCurrentState == BloodGlucoseCurrentState.INCREASING && bloodGlucose <= idealBloodGlucoseLevel)
		       {
		    	   bloodGlucose = bloodGlucose + 0.10;  
		       }
       		   if(bloodGlucose > maxChangeInBloodGlucose || bloodGlucose > 150)
       		   {
